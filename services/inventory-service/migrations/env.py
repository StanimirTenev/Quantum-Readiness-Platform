"""Alembic environment for inventory-service's PostgreSQL schema only.

SQLite (bare-metal dev/tests/CI) keeps its existing ALTER-TABLE-if-missing
pattern in app/repository.py -- this migration history exists purely for the
...
"alembic_version") because inventory-service and workflow-service share one
physical Postgres database (see infra/docker/README.md) -- two independent
migration histories in the same database would collide on the default table
name otherwise.
"""
