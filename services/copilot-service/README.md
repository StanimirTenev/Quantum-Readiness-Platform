...
- Change Assistant's pre-change checklist is a deterministic starter set keyed off the same rationale flags Risk Narrator uses, now including SSH (weak kex/host-key/cipher/MAC), IPsec/IKEv2 (legacy DH group/weak encryption/integrity/PRF), repo/IaC (embedded private key), and CI-pipeline (signing command detected -- keyed off `ci_signing_command_detected`, which flags the pipeline for manual key-algorithm review rather than judging it weak) items.
